...
exports.signup = async (req,res) => {
    ...
    const {firstName, lastName,email,password} = req.body;
        ...
        console.log(_user);
        _user.save().then(data => res.status(200).send({message:"Admin saved successfully"})).catch((err) => res.status(400).send({message: "Error is saving data"}))
}

exports.signin =  (req,res) => {
    User.findOne({email: req.body.email})
    .then(result => {
        if(result.authenticate(req.body.password) && result.role === 'admin'){
